...
function HomeComponents(props) {
    ...
    useEffect(() => {
        setProducts(props.data.obj2.length > 0 ? props.data.obj2[0] : products)
    }, [props.data])
        ...
        <React.Fragment>
            ...
            {/* <table>
                ...
                {products.map(item => (
                    <tr>
                        <td>{item.title}</td>
                        <td><img src={item.thumbnail} alt={item.name} style={{ maxWidth: '50px', maxHeight: '50px' }} /></td>
                        <td>{item.price}</td>
                        <td>{item.stock}</td>


                        <td>
                            <button onClick={() => props.addToCartHandler({ id: item.id, qty: item.qty, price: item.price, title: item.title, productData: products })} > + </button>&nbsp;&nbsp;
                            <button onClick={() => props.removeitemToCartHandler({ id: item.id, productData: products })}> - </button>&nbsp;&nbsp;
                            <button onClick={() => props.removeitemToCartHandler({ id: item.id, flag: true, productData: products })}> Remove Item</button>
                        </td>
                    </tr>
                ))}
            ...
            <Row>
                    {
                        products.map(item => (
                            ...
                            <Card style={{ width: '18rem' }}>
                                <Card.Img variant="top" src={item.thumbnail} alt={item.name} style={{height:"40%",width:"40%"}}/>
                                <Card.Body>
                                    <Card.Title>{item.title}</Card.Title>
                                    <Card.Text>
                                      Available Quantity : {item.stock}
                                    </Card.Text>
                                    <Button variant='success' onClick={() => props.addToCartHandler({ id: item.id, qty: item.qty, price: item.price, title: item.title, productData: products })} > + </Button>&nbsp;&nbsp;
                                    <Button variant='warning' onClick={() => props.removeitemToCartHandler({ id: item.id, productData: products })}> - </Button>&nbsp;&nbsp;
                                    <Button variant='danger' onClick={() => props.removeitemToCartHandler({ id: item.id, flag: true, productData: products })}> Remove Item</Button>
                                </Card.Body>
                            </Card>
                </Col>
                        ))
